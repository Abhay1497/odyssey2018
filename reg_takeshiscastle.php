...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn) {
        ...
        $sql = "SELECT max(part_id) FROM takeshiscastle";
        ...
        $sql = "insert into takeshiscastle values('$name1','$mobile1','$email1','$name2','$mobile2','$email2','$college','$id')";
        if (mysqli_query($conn, $sql) == 1) {

            //sending email
            $email_subject = "ODYSSEY Registration";
            $email_body = "Thank you for registering in Takeshi's Castle.\nYour Participation ID is ".$event_id;
            include 'email.php';
